 ...
 */
class NycMigrateBatchDocumentsController extends ControllerBase {
  ...
  public static function executeBatchProcess() {
    ...
    //$ct = $updater->getDocumentSourcesCt();
    ...
    while ($offset < (int) $ct && ((int) $ct < $limit)) {
      if (!empty($batch_node_ids)) {
        ...
        $batch_operations[] = [
          '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_documents_update',
          [$node_batch]
        ];
      }
    }
