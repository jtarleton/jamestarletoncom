 ...
 */
class NycMigrateBatchVideosController extends ControllerBase {
  ...
  public static function executeBatchProcess() {
    ...
    $batch_operations = [];
    //$batch_node_ids = $updater->getVideoSources();
    ...
    while ($offset < (int) $ct && ((int) $ct < $limit)) {
      if (!empty($batch_node_ids)) {
        ...
        if (!empty($node_batch)) {
          $batch_operations[] = [
            '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_videos_update',
            [$node_batch]
          ];
        }
      }
