 ...
 */
class NycMigrateBatchUnpublishMapsController extends ControllerBase {
   ...
   */
  public static function executeBatchProcess($limit = NULL, $offset = NULL) {
    $updater = new NycMigrateUpdater;


    $ct = $updater->getMapSourcesCt();
    $success = [];
    $results = [];
    $node_batches = [];
    $batch_size = 5;
    ...
    $batch_operations = [];
    $batch_node_ids = $updater->getMapSources();

    // 2. Retrieve all nodes of project type.
    try {
      $batch_node_ids = $updater->getSourceNodeIds($limit, $offset, 'map');
    }
    catch (\Exception $e) {
    ...
    if (!empty($batch_node_ids)) {
      foreach ($batch_node_ids as $nid) {
        $batch_operations[] = [
          '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_maps_unpublish',
          [
            $batchId, $nid,
