 ...
 */
class NycMigrateBatchVideoController extends ControllerBase {
  ...
  public static function executeBatchProcess() {
    ...
    $ct = $updater->getSourcesCt();//$updater->getVideoSourcesCt();
    $success = [];
    $results = [];
    $node_batches = [];
    $batch_size = 5;
    ...
    $batch_operations = [];
    $batch_node_ids = $updater->getSources(); //$updater->getDocumentSources();
    ...
    while ($offset < (int) $ct && ((int) $ct < $limit)) {
      if (!empty($batch_node_ids)) {
        ...
        if (!empty($node_batch)) {
          $batch_operations[] = [
            '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_video_update',
            [$node_batch]
          ];
        }
      }
