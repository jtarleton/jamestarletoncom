 ...
 */
class NycMigrateBatchProjectlinkController extends ControllerBase {
  ...
  public static function executeBatchProcess() {
    ...
    $batch_operations = [];
    $batch_node_ids = $updater->getDocumentSources();
    /*
    ...
    while ($offset < (int) $ct && ((int) $ct < $limit)) {
      if (!empty($batch_node_ids)) {
        ...
        $batch_operations[] = [
          '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_link_update',
          [$node_batch]
        ];
      }
    }
