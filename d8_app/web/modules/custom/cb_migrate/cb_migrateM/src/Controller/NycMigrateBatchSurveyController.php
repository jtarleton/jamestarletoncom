 ...
 */
class NycMigrateBatchSurveyController extends ControllerBase {
  ...
  public static function executeBatchProcess() {
    ...
    $ct = $updater->getSourcesCt(); //$updater->getDocumentSourcesCt();
    $success = [];
    $results = [];
    $node_batches = [];
    $batch_size = 5;
    ...
    while ($offset < (int) $ct && ((int) $ct < $limit)) {
      if (!empty($batch_node_ids)) {
        ...
        $batch_operations[] = [
          '\Drupal\nyc_migrate\Service\NycMigrateBatchService::nyc_migrate_do_survey_update',
          [$node_batch]
        ];
      }
    }
